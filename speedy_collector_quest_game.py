...
class Game:
    ...
    def generate_items(self):
        missing_items = ITEMS_TO_COLLECT - len(self.items)
        for _ in range(missing_items):
            while True:
                ...
                for item in self.items:
                    if self.is_collision(self.canvas.coords(item), (x, y, x + ITEM_SIZE, y + ITEM_SIZE)):
                        overlapping = True
                        break
                if not overlapping:
                    break
        ...
        for _ in range(5):
            while True:
                ...
                for obstacle in self.obstacles:
                    if self.is_collision(self.canvas.coords(obstacle), (x, y, x + ITEM_SIZE, y + ITEM_SIZE)):
                        overlapping = True
                        break
                if not overlapping:
                    break
    ...
    def move_items(self):
        if not self.game_paused:
            for item in self.items:
                ...
                if x2 < 0:
                    x = random.randint(WIDTH, WIDTH + ITEM_SIZE * 2)
                    y = random.randint(50, HEIGHT - ITEM_SIZE)
                    self.canvas.coords(item, x, y, x + ITEM_SIZE, y + ITEM_SIZE)
            for obstacle in self.obstacles:
                ...
                if x2 < 0:
                    x = random.randint(WIDTH, WIDTH + ITEM_SIZE * 2)
                    y = random.randint(50, HEIGHT - ITEM_SIZE)
                    self.canvas.coords(obstacle, x, y, x + ITEM_SIZE, y + ITEM_SIZE)
